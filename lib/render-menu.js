  ...
  return `      <template class="menu-detail">
        ...
        <div class="menu-detail-body">
          ...
          <div class="menu-chips">${notesOf(item)
            .map((n) => `<span class="menu-chip is-note"><svg class="menu-dish-note-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${n.icon}</svg>${escapeHtml(noteName(n, locale))}</span>`)
            .join('')}</div>`
            : ''}
          ${(item.diets || []).length > 0

...
const { notesOf, noteName } = require('./dish-notes');
...
function renderMenuPage(dati, locale, t) {
  const { venueName, logoUrl, accent, tableConditions, slug } = dati;
        ...
        <div class="menu-detail-body">
          ...
          ${(item.allergens || []).length === 0
            ...
            : `<div class="menu-chips">${(item.allergens || [])
                .map((c) => {
                  // L'ICONA C'È SEMPRE nel dettaglio, non solo in modalità a
                  // icone: qui accanto c'è il nome scritto, quindi il
                  // pittogramma non deve dire la cosa da solo — aiuta a
                  // riconoscerla, e mette le pastiglie degli allergeni sullo
                  // stesso piede di quelle delle note, che l'icona ce
                  // l'avevano già. Un allergene senza disegno resta la sua
                  // sola parola.
                  const d = allergenIcon(c);
                  return `<span class="menu-chip" data-kind="allergens" data-code="${escapeAttr(c)}">${
                    d
                      ? `<svg class="menu-chip-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${d}</svg>`
                      : ''
                  }${escapeHtml(labelFor(c, locale))}</span>`;
                })
                .join('')}</div>`}
          ${(item.diets || []).length > 0

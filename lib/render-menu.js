...
function renderMenuPage(dati, locale, t) {
  ...
  const conFoto = showPhotos && piatti.some((p) => (p.thumbUrl || '').trim() !== '');
...
<body class="menu-body${headingFont === 'modern' ? '' : ` font-${headingFont}`}" style="--accent:${escapeAttr(tinta(accent))};--ms:${textScale};--lh:${lineHeight}">
...
function renderGruppo(gruppo, currency, locale, t, aspetto) {
  ...
  ${(gruppo.name || '').trim() !== ''
      ? `<h2 class="menu-section-title is-${stile}">${escapeHtml(gruppo.name)}</h2>`
      : ''}
...
// qualcosa sul prezzo invece che sul piatto. Apre il dettaglio, che è dove stanno la descrizione
...
function renderRiga(item, currency, locale, t, aspetto) {
  const { conFoto, showPhotos, photoShape, showDescriptions } = aspetto;

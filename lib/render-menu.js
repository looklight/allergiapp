// La pagina che il cliente apre col QR sul tavolo: /v/[slug].
//
...
function renderMenuPage(dati, locale, t) {
  ...
  const canonical = slug ? `${SITE_ORIGIN}/v/${slug}` : null;
  ...
  const baseLingue = slug ? `/v/${slug}` : dati.langBase || null;
...
${LINGUE.map((l) => {
    const href = l.code === 'it' ? canonical : `${canonical}?lang=${l.code}`;
...
function renderHeader(dati, locale, t, baseLingue, carte, menu) {
  ...
  <div class="menu-header-top">
    ...
    ${!baseLingue ? '' : `<nav class="menu-langs" aria-label="${escapeAttr(t('menuLanguage'))}">
      ${LINGUE.map((l) => {
        const href = l.code === 'it' ? baseLingue : `${baseLingue}?lang=${l.code}`;

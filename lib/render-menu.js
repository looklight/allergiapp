...
${renderHeader(dati, locale, t, baseLingue)}
...
function renderHeader(dati, locale, t, baseLingue) {
  const { venueName, logoUrl, menu } = dati;
...
function renderNote(item, locale) {
  ...
  return `<span class="menu-item-notes">${note
    .map(
      (n) =>
        `<span class="menu-note"><svg class="menu-note-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${n.icon}</svg>${escapeHtml(
          noteName(n, locale)
        )}</span>`
    )
    .join('')}</span>`;
  ...
  return `      <template class="menu-detail">
        ...
        <div class="menu-detail-body">
          ...
          <div class="menu-chips">${notesOf(item)
            .map((n) => `<span class="menu-chip is-note"><svg class="menu-note-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${n.icon}</svg>${escapeHtml(noteName(n, locale))}</span>`)
            .join('')}</div>`
            : ''}
          ${(item.diets || []).length > 0

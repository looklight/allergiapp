...
function renderLink(links, carte) {
  const righe = (links || [])
    .map((l) => ({ href: safeHref(l.url), nome: nomeLink(l.url, l.label) }))
...
${righe
    .map(
      (l) =>
        `  <a class="menu-link" href="${l.href}" target="_blank" rel="noopener noreferrer nofollow">${renderIconaLink(l.nome)}${escapeHtml(l.nome)}</a>`
    )
    .join('\n')}
</nav>`;
}

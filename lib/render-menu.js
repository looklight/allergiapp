...
function renderMenuPage(dati, locale, t) {
  ...
  const conFoto = piatti.some((p) => (p.thumbUrl || '').trim() !== '');
...
${gruppi.length === 0
    ...
    : gruppi.map((g) => renderGruppo(g, menu.currency, locale, t, conFoto)).join('\n')}
...
// non è.
function renderGruppo(gruppo, currency, locale, t, conFoto) {
  if (gruppo.kind === 'note') {
  ...
  <ul class="menu-items">
${gruppo.items.map((item) => renderRiga(item, currency, locale, t, conFoto)).join('\n')}
  </ul>
</section>`;
}
...
function renderRiga(item, currency, locale, t, conFoto) {

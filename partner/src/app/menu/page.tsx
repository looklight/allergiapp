...
export default function MenusPage() {
  ...
  const { showcases, create: createVenue } = useShowcases();
  ...
  async function handleCreate(venueId: string | null, venueName: string, menuName: string) {
    setCreating(false);
    let id = venueId;
    if (id === null) {
  ...
  const conMenu = (showcases ?? []).filter((v) =>
    (menus ?? []).some((m) => m.showcaseId === v.id)
  );
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        // Nessun menù: non si chiede più di creare prima una vetrina, il
        ...
        <div className="max-w-xl space-y-8">
          {conMenu.map((showcase) => (
            <div key={showcase.id} className="space-y-3">
              ...
              {menus
                .filter((menu) => menu.showcaseId === showcase.id)
                .map((menu) => {
      ...
      {creating && (
        <NewMenuDialog
          venues={showcases ?? []}
          hasMenus={(venueId) => (menus ?? []).some((m) => m.showcaseId === venueId)}

// Categorie piatti: SET FISSO tradotto, mai testo libero.
//
// Perché fisso, visto che ogni tanto la domanda torna: la categoria è
// l'INDICE COMUNE — «Primi» vuol dire la stessa cosa in ogni ristorante del
// mondo, quindi possiamo tradurla noi una volta per tutti e l'app può
// confrontare i locali fra loro. Il nome che parla di QUEL locale («I nostri
// fritti») è la SEZIONE del menù, che è testo libero e vive altrove
// (partner_menu_sections). Una classificazione e un indice non sono la stessa
// cosa: v. la nota in testa a partner_dishes nella migration 700.
//
// La personalizzazione c'è lo stesso, e passa da hidden_dish_categories sul
// profilo (713): chi non fa primi li nasconde e non li vede più nelle tendine.
//
// QUINDICI LINGUE dal primo giorno, come constants/allergens.ts e
// constants/diets.ts dell'app. Il portale ne mostra due perché la sua
// interfaccia parla italiano e inglese, ma il giorno che la scheda del locale
// arriva davvero nell'app le trova già scritte.
//
// I codici devono combaciare col CHECK di partner_dishes.category (700, esteso
// dalla 713).
...
// Il portale parla due lingue: la firma resta quella di prima per non toccare
// i punti che la chiamano. Una lingua che non conosciamo ripiega sull'inglese,
// mai sul codice grezzo.
...
// eccezione, nascondere «Primi» dopo aver classificato dieci paste le
// renderebbe di colpo senza categoria agli occhi del ristoratore, che non
// capirebbe né perché né come rimediare.
export function visibleCategories(hidden: string[], inUse: string[] = []): DishCategory[] {
  return DISH_CATEGORIES.filter((c) => !hidden.includes(c.code) || inUse.includes(c.code));
}

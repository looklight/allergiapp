...
**Portale partner — la migration 700 è APPLICATA (2026-08-30), il portale però scrive ancora in localStorage:**
- [ ] **PRIMA DI TUTTO: scrivere il profilo partner in `partner_accounts`.** Oggi vive nei metadati dell'utente (`lib/partnerProfile.ts`) e la tabella è a zero righe, ma **tutte** le tabelle partner hanno la chiave esterna verso `partner_accounts(user_id)`: finché quella riga non esiste, ogni inserimento di vetrine o piatti fallisce per violazione di chiave esterna. È il primo passo dello scambio, non un dettaglio successivo. Tocca i due percorsi di iscrizione (`login/page`, `PartnerOnboarding`) e `hasPartnerProfile`, che da cancello di percorso diventa finalmente cancello vero protetto da RLS
- [ ] **Scambiare il livello dati da localStorage a Supabase.** È il passo che sblocca tutto il resto ed è confinato a tre file — `lib/storage.ts`, `lib/showcases.ts`, `lib/dishes.ts` — perché nessun componente sa che localStorage esiste. Le tabelle ci sono già: `partner_dishes` (catalogo del partner), `partner_showcase_dishes` (quali piatti accesi in quale vetrina), `partner_dish_translations`, `partner_showcases`, `partner_links`. **Niente importatore**: i dati del prototipo sono prove, si riparte vuoti (deciso 30/08). Cambia una cosa nella forma: oggi le letture sono sincrone e istantanee, con il database diventano asincrone, quindi gli stati di caricamento che ora durano un istante diventano reali
- [ ] **Far vedere gli errori di scrittura.** Dal 30/08 il portale scrive su Supabase in modo ottimistico: l'interfaccia si aggiorna subito e la scrittura parte dietro. Se fallisce — rete assente, sessione scaduta, RLS — **non se ne accorge nessuno** finché non si ricarica la pagina e il dato non c'è più. Con dati di prova è tollerabile, col menù di un ristoratore no. Serve almeno un avviso che dica "non è stato salvato" e una riprova
- [ ] **Foto dei piatti su Supabase Storage.** Oggi sono data-URL dentro al browser: un catalogo vero non ci sta. Da fare insieme allo scambio o subito dopo, la colonna `photo_url` esiste già

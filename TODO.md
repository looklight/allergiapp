...
- [ ] ⚠️ **Applicare la migration 086 (contatori con dimensione)** PRIMA di pushare main (le tre partner sono già applicate: resta solo questa). Crea `daily_dimension_counters` + `bump_daily_dimensions` + `bump_card_open`. Innocua da sola: le RPC restano lì senza chiamanti finché non esce la build. Il contrario no — l'app che chiama una funzione inesistente fallirebbe in silenzio a ogni apertura della card.
...
- [ ] ⚠️ **Ripuntare `get_top_filtered_needs` (mig 503) sui contatori anonimi** (branch `admin-prod`, dopo la build con la mig 086) — l'app non manda più l'elenco delle esigenze dentro l'evento `filter_applied` (era un dato sanitario legato all'id utente): il dettaglio ora sta in `daily_dimension_counters` con nome `filter_need`. Senza questa modifica il widget resta fermo ai dati storici senza dare errore. Nella stessa passata: due widget nuovi da `card_need` e `card_language` (quali esigenze e quali lingue sulle card). ⚠️ Presentare le esigenze come **presenze, non percentuali di utenti**: la somma è più alta del numero di aperture perché ognuno ne ha diverse. E i numeri di `filter_need` saliranno di colpo rispetto alla serie vecchia, perché il contatore non passa dal consenso e conta tutti.
...
- [ ] **Heat map delle aperture di scheda** (branch `admin-prod`, nessun lavoro sull'app) — `restaurant_view_counts` (mig 082) esiste dal 30/08 e ogni ristorante ha `location`: aperture + coordinate = mappa di calore delle zone più consultate. Dati retroattivi da fine agosto. Limite da dichiarare nel widget: conta aperture, non persone.

...
- [ ] **Collegamento locale ↔ ristorante — DESIGN CHIUSO il 17/09**, in `MONETIZATION.md` «Associazione locale ↔ ristorante». Ordine: 1) database · 2) portale (ricerca, conferma con mappa, P.IVA + dichiarazione, scollega, pausa) · 3) admin (azioni, richieste contese, registro) · 4) app, con la build nativa (scheda, contorno pin, risposte). Le parti 1-3 non chiedono build.
  ...
  - [ ] ⚠️ **Prima che il portale con l'associazione vada online**: nell'informativa (sezione Ristoratori, `landing`) una frase che dice che la P.IVA si verifica su **VIES** (Commissione europea) e che l'esito si conserva per verificare chi parla per il ristorante. Attenzione: in `landing` ci sono modifiche non committate di un'altra sessione su `translations.json`.

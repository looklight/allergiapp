...
- [x] ~~**La UI della scatola "Aspetto"**~~ — RIFATTA il 2026-09-06 in **tre gruppi** (La carta · L'identità · Il testo), con «La carta» a tutta larghezza e i due gruppi piccoli affiancati sotto. Diario: Tema 33; tecnica in `partner/README.md`. La valuta è uscita dai gruppi (non è aspetto) e sta in fondo staccata.
  - [x] ~~**Spostare la valuta nell'area Contenuto**~~ — SCARTATO il 2026-09-16: l'utente la vuole dove sta, in fondo alla scatola dell'aspetto. Non riproporlo.
...
- [ ] **Parte 4 — l'app** (esce con una build nativa; le OTA sono bloccate, quindi si accumula su `main`).
  ...
  - [x] **La schermata — SCRITTA il 2026-09-21, MAI VISTA GIRARE** (serve una dev build): carosello di max 8 piatti + «+N · Vedi tutto», pill dei link, riordino per compatibilità **nell'app** (le esigenze di chi guarda non si mandano al database: sono dati sanitari). Il modello è `SchedaPreview` nel portale, che è già la replica fedele della scheda.
...
- [ ] **Migration 732 — i link si salvano in un gesto solo** (`732_partner_save_links.sql`, scritta il 21/09, **DA APPLICARE prima di pushare il portale**: senza la funzione il salvataggio dei link non funziona più). Il portale faceva due viaggi (cancella tutti i link del locale, poi riscrivi) e fra l'uno e l'altro il locale era **senza link**: se il secondo falliva, quello restava lo stato finale. È successo il 21/09 con l'errore `sort_order` (PostgREST in una scrittura multipla unisce le colonne di tutte le righe e mette NULL dove manca il campo, invece del valore di partenza della colonna). Ora `partner_save_links(locale, link)` fa delete+insert in una transazione, SECURITY INVOKER (la difesa resta la policy `partner_links_owner` della 703), e scrive le colonne una per una: la trappola di PostgREST non si ripresenta.
...
- [x] **Migration 086 APPLICATA** l'11/09/2026 (contatori con dimensione: `daily_dimension_counters`, `bump_daily_dimensions`, `bump_card_open`). I contatori restano a zero finché non esce la build che li chiama.
- [ ] **Cancellare i dati sanitari già raccolti su Firebase/GA4** (azione manuale, console Google): il fix nel codice ferma il rubinetto, non svuota la vasca. Rimuovere le dimensioni personalizzate registrate (`allergens`, `diet_modes`, `other_foods`, `vegetarian_level`, `restriction_count`, `allergen_count`) e richiedere la cancellazione dei dati utente. La conservazione di default è 14 mesi, quindi senza intervento restano fino a fine 2027.
...
- [x] **Policy — email non più "solo per accedere"** — FATTO 2026-09-06 (landing `d697bc4`, main `059e300`). Oggi la policy dice che l'email serve *solo* per accedere: ci auto-vincola più di quanto serva, e un'email di servizio a un utente (chiarimento su una recensione, moderazione) risulterebbe fuori dalla finalità dichiarata. Testo nuovo, stessa lunghezza dell'attuale:
...
### Mappa — rettangolo oltre ~200° di longitudine: la mappa si svuota (trovato 2026-09-05, NON corretto)
Difetto reale e silenzioso, trovato sul branch `feature/map-thinning` e rimasto lì quando il branch è stato abbandonato: **va rifatto su main**.

- [ ] Castato a geography, PostGIS legge un envelope più largo di ~200° di longitudine come "tutto il resto del globo" e la RPC torna quasi vuota, **senza alcun errore**. Misurato: 270° → 113 righe, 300° → 6, 355° → 0.
- Telefono in verticale non ci arriva (~110° al massimo), **iPad in orizzontale sì**: la mappa si svuoterebbe di colpo a zoom largo senza che nulla lo segnali.
- Cura (una riga in `hooks/useRestaurantGeo.ts`, nel calcolo dei bounds): clampare il mezzo-span di longitudine a 90°, `const halfLng = Math.min(lngDelta / 2 + margin, 90)`. A quella scala comanda comunque il tetto delle 1000 righe, quindi non si perde niente che si sarebbe avuto.
...
**Fatto accertato il 2026-09-05, da tenere presente prima di allarmarsi sulla crescita: il lag NON peggiora aggiungendo ristoranti.** Il numero di marker montati è limitato da tre tetti che non dipendono dall'archivio — risposta del server troncata a 1000 righe (`PIN_RESPONSE_CAP`), pinCache sfoltita sopra 4500 → 3000 (`useRestaurantGeo.ts:436`), cache dettagliata a 1000 (`CACHE_MAX_SIZE`). Con 4.000 locali o con 400.000 il telefono ne disegna lo stesso ordine di grandezza: **il lag di oggi è già il caso peggiore**. Quello che peggiora crescendo è la *quota di archivio mostrata* (24% oggi, 2,4% a 10× i dati), cioè un problema di completezza e non di prestazioni. Il collegamento fra i due: togliere il tetto (tile) fa tornare il conto dei marker a dipendere dall'archivio, ed è per questo che tile e culling vanno fatti nella stessa finestra.
...
- [x] ~~**`LIMIT` pallini 1000→3000**~~ — **superato dalla migration 085 (applicata 2026-09-05)**. Il problema non era il valore del tetto ma il fatto che il taglio fosse *cieco*: seguiva l'ordine fisico della tabella, in pratica italiano. Ora il rettangolo si divide in una griglia 31×31 e passa prima un locale per quadretto abitato (31×31 = 961 < 1000: il primo giro non può mai essere troncato), poi il resto per densità. Misurato fianco a fianco: nordici 16→23, paesi rappresentati 27→35, italiani 778→725. Nessuna build, agisce sulla 1.3.1 già negli store. Dettagli e manopola copertura/densità (`LEAST(rn, 2)`) in `MAP_SCALING.md` §0-quater.
...
- [ ] **Aggregazione server-side a griglia / tile (`get_map_aggregates`) — resta il prossimo lavoro vero, ma non più urgente (agg. 2026-09-05).** La 085 ha tolto l'*iniquità* del taglio, non il taglio: a zoom largo arriva ancora **il 24% dell'archivio** (1000 righe su 4106), solo distribuito bene invece che tutto italiano. Con la crescita la quota scende comunque — a 10× i dati sarebbe il 2,4% — ma la garanzia della 085 tiene: ogni quadretto abitato ha sempre un rappresentante, quindi la mappa resta **geograficamente onesta e informativamente povera** invece di sembrare vuota. Questo compra tempo, non risolve.
  ⚠️ **Da pianificare INSIEME al viewport culling, non prima né dopo.** Oggi il numero di marker montati è limitato dal tetto delle 1000 righe, quindi il lag non peggiora crescendo (v. il "fatto accertato" in testa a questa sezione). I tile tolgono quel tetto → al client arrivano molti più dati → il conto dei marker torna a dipendere dall'archivio. Fare i tile senza il culling significa risolvere il vuoto e riaprire il lag, peggio di com'è ora.
  Chiude in un colpo cinque cose: completezza della mappa, ~20% di egress, il limite delle 1000 righe (le celle cappano per costruzione), la visibilità dei premium a zoom largo, e il payload che cresce col DB.
  Design chiuso in `MAP_SCALING.md` §0 (grid-dots); §0-ter per cosa è cambiato scrivendo il passo A e **§0-quater per l'esito** (la 085, le lezioni di metodo, e cosa NON riprovare). **Da decidere DENTRO il design, non dopo**: filtro cucina sopra le celle, filtro "Recensiti dai seguiti" sopra le celle, e il canale densità (pallino più grosso dove ce n'è di più) che è la leva di "pienezza" chiesta dall'utente. Finestra naturale anche per le colonne precalcolate su `reviews` (punto 4-bis: da sole hanno ~4 anni di margine al ritmo attuale, ~900 recensioni/mese, quindi NON vanno anticipate).
...
### Egress Supabase — ottimizzazioni censite (misurazione 2026-08-10)

**Nessuna di queste è urgente oggi**: sono censite qui per decidere *quando* agire, non per farle subito. Contesto costi e trigger per il piano Pro in memoria `project_supabase_plan_backup.md`.

**Come sono stati ricavati i numeri.** Payload misurati chiamando le RPC reali su produzione (byte gzip sul filo — le risposte escono compresse, `content-encoding: gzip`, quindi è quello che Supabase conta) e incrociati con i volumi d'uso reali: DAU 220-284 (`daily_counters.active_users`), 1.282 `restaurant_viewed`/giorno, 138 ricerche/giorno (`analytics_events`, finestra 11/07-10/08). Il modello dà **~70 MB/giorno ≈ 2 GB/mese su 5 GB free**, coerente col 44% osservato a luglio sulla pagina Usage. **Da ri-misurare prima di agire** (gli script stanno solo nella sessione, vanno rifatti: ~20 min).

Ripartizione: **pin mappa ~50%**, foto recensioni ~24%, liste ristoranti ~10%, resto (auth/profilo/community/traduzioni) ~10%, schede ristorante ~3% (1,5 KB gzip per apertura: già magrissime, non toccarle).

**La distinzione che conta per il futuro.** I pin sono rendibili *strutturalmente limitati*: con l'aggregazione a celle il payload dipende da quante celle stanno a schermo, non da quanti ristoranti ci sono nel DB — si passa a 100k ristoranti e il costo per fetch non si muove. Le foto no: crescono con `aperture scheda × copertura foto × foto per recensione`, e oggi la copertura è bassa solo perché la base è giovane (18% delle recensioni ha foto, 1,7 in media). A **parità di utenti di oggi**, se la copertura sale al 50% con 2 foto siamo a ~6,7 GB/mese; a saturazione (100%, il tetto di 3 foto per recensione) ~20 GB/mese. **Sono le foto a sfondare il free tier prima degli utenti**, ed è per questo che la voce sul prefetch qui sotto conta più del suo guadagno immediato.

Vincoli comuni: OTA bloccate (le voci client escono con la prossima build nativa) e **le RPC esistenti non si modificano nella forma** — i client 1.3.0 in store le usano; ogni proiezione diversa va su una RPC nuova.

- [ ] **Aprire la mappa sull'ultima regione vista** (~20% dell'egress totale, ~13 MB/giorno). Oggi ogni mount fa un fetch incondizionato su `DEFAULT_REGION` (delta 22° = mezza Europa) = 42 KB gzip fissi, e se poi il GPS risolve ne parte un secondo attorno all'utente. Persistendo l'ultima regione (AsyncStorage) il fetch di apertura diventa da città (~6 KB). `hooks/useRestaurantGeo.ts:521` (l'effect di mount) + `lastRegionRef` già esistente. **Non è un compromesso: è anche UX migliore** (la mappa riapre dove eri). Lavoro: ~1h.
- [ ] **Thumbnail solo di quello che si vede** (guadagno oggi modesto, ~7%; domani è il freno principale). `hooks/useRestaurantDetail.ts:160` fa `Image.prefetch` delle thumbnail di **tutte** le 15 recensioni della prima pagina, anche se l'utente ne scorre tre. Tetto per apertura scheda: 15 × 3 foto × 11,6 KB = **522 KB**, 40× la media attuale. Caricando solo le thumbnail visibili (lazy nella FlatList) il costo per apertura resta 3-6 thumbnail (~35-70 KB) qualunque sia la copertura futura — è quello che rende le foto limitate come i pallini. Lavoro: ~2h.
- [ ] **`cacheControl` lungo sull'upload foto** (~7%, una riga). `services/storageService.ts:79` non passa `cacheControl`, quindi le foto escono col default `max-age=3600`: il device riscarica le thumbnail dopo un'ora. Con 7-30 giorni spariscono quasi tutti i riscarichi. **Caveat da chiudere insieme**: sostituendo una foto il path resta identico (`{reviewId}_{index}.webp` con `upsert: true`) → la vecchia resta in cache fino a scadenza. Si risolve caricando la sostituzione su un path nuovo e cancellando il vecchio. Vale solo per i file caricati dopo la modifica. Lavoro: ~30 min.
- [ ] **Proiezione più magra sulla lista nearby** (~4%). `get_nearby_restaurants` proietta anche `subscription_status`, `subscription_expires_at`, `updated_at`, `is_premium` — verificato: zero usi nella UI, ~20% del peso della riga (49 KB raw per 66 risultati). Va fatto su una RPC nuova, non modificando l'esistente (romperebbe le 1.3.0 in store). Basso valore da solo: da accorpare alla prossima modifica che tocca comunque quelle RPC (es. il sort server-side della sezione "Nearby" sopra). Lavoro: ~1h.
- **Aggregazione server-side a griglia** (`get_map_aggregates`, MAP_SCALING.md §4, già a task sopra): oltre alla scalabilità dei marker vale **un altro ~20% di egress**, perché a zoom largo oggi scarichiamo 1000 righe di pin completi per disegnare dei puntini che quei dati non li usano. Motivazione in più per lo stesso lavoro, non un task nuovo.
- **Max Rows a 3000 — argomento in più per il NO** (decisione già presa il 2026-07-12, sezione "Mappa — follow-up 2026-07-10" sopra): triplicherebbe il payload a zoom largo (42 → ~125 KB gzip a fetch) e da solo porterebbe l'egress oltre il free tier. Se e quando si farà l'aggregazione a celle, il problema che avrebbe dovuto risolvere sparisce.
- Nota: **lo storage in sé non è un vincolo** — 67 MB su 1 GB gratis (1.329 foto, +~150 KB/giorno). Il tetto che morde è solo l'egress.
...
- **Menu digitale** — **l'editor ESISTE dal 2026-08-31** (`/menu` nel portale partner, migrations 703/704 applicate): sezioni, prezzi, riordino trascinando, anteprima col filtro allergeni. Direzione e 17 decisioni in `DIGITAL_MENU.md`. ⚠️ **Non è un premium**: il Tema 2 dice che il filtro è la dimostrazione del prodotto e non si chiude, e il Tema 10 che i gratuiti saranno la maggioranza ed è da loro che arriva il contenuto sugli allergeni. La sezione foto menù community è stata **rimossa** dall'app (commit `32c6d61`, esce con 1.3.1). Il `MenuLink[]` del portale (link a un menù esterno) resta una cosa diversa e continua a valere.
  - [x] ~~**Rinomina "vetrina"**~~ — FATTA 31/08. Il contenitore è il **locale** (`Venue`, `src/lib/venues.ts`, rotta `/locale/[id]`), quello che finisce nell'app è la **scheda AllergiApp**; "vetrina" non compare più in nessuna delle due lingue. L'aveva resa urgente un'incomprensione vera: la stessa riga si chiamava "Nome della vetrina" nell'editor (con placeholder *"come vuoi chiamarla"*) e "Nome del locale" nel menù, dove però quel nome lo leggono i clienti al tavolo
  - [x] ~~**Eliminare un locale portava via i menù in silenzio**~~ — FIXATO 31/08 insieme alla rinomina: cascata della 704, la conferma diceva solo "i link andranno persi" e l'annulla non li rimetteva. Ora si contano nella finestra e l'annulla li ripristina con gli stessi id
  - [x] ~~**Gli interruttori "Sulla scheda" in `/piatti` non fanno niente** e non lo dicono~~ — FATTO (verificato 2026-09-02): dove la scheda non c'è il comando non c'è, e al suo posto si legge il motivo. Filtro `cardId !== null` in tre punti (`venuesConScheda` in `/piatti`, `venue.cardId === null` nell'editor del locale, `conScheda` in `DishPanel`), con la guardia in `setDishOn` come ultima rete
...
- [ ] **Verifica dei link lato server alla pubblicazione della scheda**: dal browser del partner non è possibile (CORS), quindi controllo alla pubblicazione con esito visibile in admin. Nel portale c'è già `normalizeUrl` che completa lo schema mancante (`www.x.it` → `https://www.x.it`), ma non dice niente su dominio sbagliato o pagina rimossa

**Portale partner — 700, 701 e 702 APPLICATE; il portale scrive su Supabase dal 30/08 e le foto stanno sullo Storage dal 31/08:**
- [x] ~~**Scrivere il profilo partner in `partner_accounts`**~~ — FATTO 30/08 (migration 701 per il consenso marketing, `hasPartnerProfile` sostituito da una lettura della tabella).
- [x] ~~**Scambiare il livello dati da localStorage a Supabase**~~ — FATTO 30/08. Vetrine, piatti, link, traduzioni e accostamenti sono su Supabase; verificato da due dispositivi diversi. L'editor salva dopo una pausa di battitura (800 ms) invece che a ogni tasto, con salvataggio forzato all'uscita.
- [x] ~~**Far vedere gli errori di scrittura in interfaccia**~~ — FATTO 31/08. Ogni scrittura passa da `write()` in `lib/saveState.ts` (stato condiviso fuori da React, perché le scritture partono dal livello dati e l'avviso deve sopravvivere al cambio di schermata). `SaveStatus` nella Shell: pill discreta in alto a destra per "Salvataggio… / Salvato", **fascia rossa in cima** con "Riprova" quando qualcosa è stato rifiutato — sopra le finestre, così un pannello aperto non la nasconde — più l'avviso del browser se si chiude la scheda con roba non salvata. `write()` non lancia mai (un'eccezione salterebbe le istruzioni successive) e una scrittura nuova sostituisce quella fallita con la stessa etichetta, così "Riprova" non rimette indietro dati superati. **Da provare a mano**: DevTools → Network → Offline, scrivere nell'editor, tornare online e premere Riprova
- [ ] **Restano fuori dall'indicatore, di proposito**: il caricamento della foto (ha già il suo stato dentro la maschera) e la cancellazione dei file (un file rimasto sono byte, non dati persi). Resta invece un buco vero: le scritture ancora in volo quando si chiude la scheda partono e basta — `useDebouncedSave` fa `flush()` su `pagehide` ma non aspetta l'esito, e a quel punto non c'è più nessuno a cui dirlo
- [x] ~~**Provare le tre cancellazioni dei file**~~ — FATTE 31/08 nel browser, contro il database vero: (a) foto sostituita → la vecchia coppia resta finché la riga non è scritta, poi sparisce; (b) foto caricata e maschera chiusa con ✕ → il file caricato se ne va, quello esistente resta; (c) piatto eliminato con annulla scaduto → i suoi due file spariscono; (d) eliminato e **annullato** entro la finestra → il piatto torna **con la foto ancora al suo posto**. Nota per chi riproverà: sondare l'indirizzo pubblico con `fetch` NON dice la verità, la CDN continua a servire una copia in cache per un pezzo — va guardata `storage.objects`
- [ ] Del ritaglio è stata provata solo la foto **orizzontale**: mancano la verticale e la già quadrata, che non deve avere niente da trascinare
- [ ] **Provare a mano le foto dei piatti** (codice del 31/08 mai eseguito). Prima il **ritaglio**: scegliendo il file si apre la finestra con la foto intera, la parte esclusa in ombra e il quadrato da trascinare su un asse solo (frecce da tastiera, Esc per uscire; lo sfondo NON chiude, di proposito) — provarlo su una foto orizzontale, una verticale e una già quadrata, che non deve avere niente da trascinare. Poi i cinque percorsi, guardando su Storage in `partner` → cartella col proprio user id → `dishes/`: (1) piatto nuovo con foto → due file, uno normale e uno `_thumb`; (2) foto sostituita → restano due file soli, i vecchi spariti; (3) foto caricata e maschera chiusa con ✕ o Esc senza salvare → niente di nuovo su Storage; (4) piatto eliminato e Annulla entro 8 s → il piatto torna **con** la foto; (5) eliminato lasciando scadere → i suoi due file spariscono. Più i messaggi: un file che non è un'immagine (PDF rinominato `.jpg`) e una foto sopra i 10 MB. **Le foto già in tabella sono ancora data-URL** e restano tali finché non si ricaricano: si vedono, ma non hanno miniatura e su Storage non c'è nulla di loro
- [ ] **Passata di pulizia dei file orfani — VALUTATA E RIMANDATA (31/08), non riproporre come pulizia del codice.** Sostituirebbe le tre cancellazioni sparse con una regola sola ("cancella i file che nessun piatto usa da più di qualche minuto") e coprirebbe anche gli orfani che oggi non prendiamo: scheda chiusa al momento sbagliato, creazione o modifica rifiutata. **Non si fa ora perché i due disegni sbagliano in modo diverso**: oggi il caso peggiore è un file inutilizzato, cioè byte; la passata cancella per ASSENZA, quindi il suo caso peggiore è portare via la foto di un piatto vero se l'elenco dei piatti torna incompleto. E non è un'ipotesi teorica in questo progetto: con i pin della mappa PostgREST troncava le risposte a 1000 righe senza che nessuno se ne accorgesse (v. `MAP_SCALING.md`). **Innesco**: il primo partner vero, e va fatta **lato server** con la chiave di servizio, dove l'elenco è autorevole e non passa da una lettura del browser che può tornare a metà. Stesso innesco del backup e delle condizioni d'uso
- [x] ~~**La grandezza dei testi del menù al tavolo — la colonna c'è, l'interfaccia no.**~~ — FATTA il 2026-09-02, nessuna migration: pacchetti Compatta/Normale/Ampia nella scatola Aspetto, e la pagina pubblica li rende. Non è una classe per grandezza ma un numero solo (`--ms` sulla radice, ogni misura è `calc(Npx * var(--ms, 1))`), con i fattori in `TEXT_SCALE_FACTORS` (`partner/src/lib/venues.ts`) e la copia gemella in `landing/lib/render-menu.js`. ⚠️ Il pavimento della riga degli allergeni è nel CSS (`max()` su `.riga-minuta`), scritto in tutt'e due le copie, e sotto le tre scelte il portale lo dice anche al ristoratore
- [ ] **Guardare l'editor del menù su un telefono vero.** Il 02/09 la forma sotto `sm` è stata rifatta facendo i conti a 375px — riga del piatto su due livelli, avviso in cima a tutta larghezza, maniglie di trascinamento tolte, tendina "Sposta in" sempre visibile, bersagli da 28-32px — ma **calcolati dalle classi, mai visti renderizzati**. Da provare col dito, non solo restringendo la finestra: quello che si vuole vedere è che il nome del piatto si legga e che la frase sugli allergeni non pubblicati non finisca sotto i puntini
- [ ] **Ingrandire la foto del piatto premendola.** Il quadrato da 900px c'è ed è caricato apposta, ma oggi non lo mostra nessuno: nel portale si vedono solo miniature in cerchi da 44-64px. Serve per il portale e ancora di più per la pagina pubblica del menù
- [ ] **Sessione nei cookie con `@supabase/ssr`.** Oggi il server non sa chi è l'utente: manda a tutti la stessa pagina vuota con scritto "Caricamento…", e solo dopo il JS scopre se sei loggato. Vale ~0,3-0,8 s all'apertura e toglie `@supabase/supabase-js` (~50 KB compressi) dal percorso di avvio della home, perché l'autenticazione la farebbe il server. **Ora il vincolo non è più la 700 ma lo scambio del livello dati**: finché le vetrine stanno in localStorage il server non può disegnare il contenuto lo stesso, quindi si incasserebbe una frazione del beneficio e si rifarebbe il lavoro. L'auth tocca solo 4 file (`AuthGuard`, `login/page`, `account/page`, `PartnerOnboarding`), è un lavoro contenuto quando sarà il momento
- [ ] Dopo lo scambio, e solo se serve: valutare di togliere realtime/storage dal bundle Supabase. Prima no, serve il client del database
- [ ] **Ripulire i residui partner dai metadati dell'utente** (`account_type`, `first_name`, `last_name`, `terms_accepted_at`, `marketing_consent`): dal 30/08 il profilo vive in `partner_accounts` e quelle chiavi non le legge più nessuno, ma restano una seconda copia che può divergere. Non urgente e non espone niente; il rischio vero è che qualcuno in futuro le rilegga "per non fare una query" e prenda dati vecchi. Da fare dal client sulla sessione dell'utente quando si toccheranno di nuovo `login/page` e `PartnerOnboarding` — **non** con una modifica diretta al record di autenticazione: lì dentro ci sono anche `display_name`, `email_verified` e `sub`, che sono di Supabase e dell'app
- [ ] **Condizioni d'uso e informativa privacy del portale — RIMANDATE AL LANCIO (deciso 2026-08-30).** Il modulo di iscrizione fa accettare due documenti che non esistono ancora. Non è un debito che matura finché gli unici iscritti siamo noi: il consenso raccolto è il nostro. Scriverli è lavoro di lancio e si fa al lancio. **Innesco: il primo ristoratore vero** — lo stesso dei backup, e col menù digitale gratuito arriva prima di quanto sembri, perché per iscriversi non deve pagare niente. Da quel momento la casella "accetto le condizioni" deve puntare a qualcosa
- [ ] **Backup del database prima del primo ristoratore vero.** Finora rimandato con motivo (dentro c'erano solo dati ricreabili), ma il menù digitale anticipa il momento: appena un ristoratore compila il suo menù, dentro c'è il *suo* lavoro, non dati nostri. Il trigger concordato ("primo premium") va spostato a "primo partner con contenuti veri", che con un piano gratuito arriva molto prima
- [ ] **Guardia in `supabase/functions/delete-account`**: oggi fa `auth.admin.deleteUser`, che cancellando l'utente dell'app porterebbe via anche il profilo partner della stessa persona (segnalato dalla 700 stessa)
...
- [ ] **Visualizzare i contatori anonimi in admin quando fluiscono i dati** (dopo la prima build che include mig 082 client-side, branch `admin-prod`, contesto in memoria `project_anonymous_counters.md`) — (a) `card_opened` da `get_daily_counters('card_opened', N)` (RPC 507 già live, generica): counter/mini-grafico in dashboard; (b) aperture anonime per-ristorante da `restaurant_view_counts`: serve una piccola RPC admin nuova (es. estendere `get_restaurant_view_stats` della 506 con i totali anonimi) + riga nella scheda locale accanto ad "Aperture" consenzienti. Da fare insieme alla card Follow qui sopra.
- [x] **`get_top_filtered_needs` ripuntata sui contatori — FATTO 11/09** (mig **509 APPLICATA**, `admin-prod` `cd2b0c8`). Torna DUE colonne invece di una: `use_count` resta l'evento (solo consenzienti, serie storica) e `anon_count` il contatore anonimo, che parte da zero. Non si sommano: la prima si svuoterà mentre la seconda si riempie, ed è un passaggio di testimone — spiegato nelle «i» delle colonne. Aggiunta anche la sezione «Esigenze e lingue delle card» (`card_need` + `card_language`), con le percentuali sulle APERTURE e non sugli utenti.
- [ ] **Andamento nel tempo di esigenze e lingue — NON prima di 2-3 mesi dalla build** (branch `admin-prod`). Lo storico si sta già accumulando da solo: `daily_dimension_counters` tiene una riga per chiave E PER GIORNO, quindi la serie temporale esiste per costruzione e non scade (a differenza di Firebase, che cancellava a 14 mesi). Manca solo la lettura: una RPC tipo `get_dimension_series(nome, chiavi[], giorni)` + un grafico a linee. ⚠️ **Mostrare la QUOTA, non il valore assoluto**: dividendo per le aperture di card dello stesso giorno (`daily_counters.card_opened`, già disponibile). Senza normalizzare, tutte le curve salgono insieme e staresti guardando la tua crescita di utenti travestita da analisi degli allergeni. Domande a cui risponde: quali lingue crescono (= dove andare a vendere i menù), come cambia il mix delle esigenze, stagionalità. Rimandato di proposito: disegnarlo contro una tabella vuota vuol dire indovinare quante serie servono e se la granularità giusta è il giorno o il mese.
- [x] **Mappa delle aperture di scheda — FATTA 11/09** (mig **510 APPLICATA**, `admin-prod` `81aba9c`). Non in dashboard ma **dentro la mappa a schermo intero della pagina Ristoranti**, come seconda modalità (Locali / Aperture): una mappa si esplora, in coda a una pagina di numeri si scorre oltre. Cerchi proporzionali (area = radice delle aperture) raggruppati per vicinanza SULLO SCHERMO, non in km, con scala ricalcolata su ciò che si vede. ⚠️ Oggi disegna quasi nulla: il contatore era attaccato alla schermata sbagliata (104 aperture contro 33.526 eventi) — corretto su main `ca03465`, diventa leggibile con la prossima build. Vedi memoria `project_anonymous_counters.md`.
...
- [ ] **Filtrare `allPins` al viewport corrente prima del render (viewport culling)** — oggi `allPins` accumula i pin di tutte le aree visitate e finiscono TUTTI in marker nativi, a schermo o no: in città si montano ~2900 marker dove ne bastano ~100. **È la causa principale del lag segnalato dagli utenti**, ed è tuttora aperto su main.
  - **Tentato e abbandonato** sul branch `feature/map-thinning` (2026-09-05): funzionava, ma il branch portava anche una griglia di diradamento a zoom largo con una regressione mai capita ("alcuni pallini spariscono" su device, sei ipotesi escluse). Si è scelto di ripartire da main pulito invece di correggere. Il branch resta nel repo, non unito: dentro c'è `mapGrid.ts` (geometria pura + 33 casi di prova) se un giorno servisse.
  - **Il culling da solo, senza la griglia, è la parte che vale**: è dove stanno gli utenti (zoom ravvicinato) ed è la parte semplice e non rotta. Non è un cambiamento visivo — con margine ~3× viewport non sparisce niente che fosse visibile.
  - ⚠️ **Non è un quick win**: reintroduce churn di mount/unmount nel punto fragile di rn-maps (churn-crash iOS, sfarfallio Android). Oggi l'insieme dei marker **solo cresce** → zero smontaggi → zero rischio. Collaudo su telefono fisico obbligatorio, non simulatore.
  - Analisi in `MAP_SCALING.md` §6 e §0-quater (che elenca anche i difetti veri trovati sul branch e ancora presenti su main).

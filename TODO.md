...
- [ ] **Aggregazione server-side a griglia (`get_map_aggregates`) — DA APPROFONDIRE, è il prossimo lavoro vero (deciso 2026-09-05).** Il trigger è scattato e il difetto è già visibile, non più teorico: a zoom largo arriva **il 24% dell'archivio** (1000 righe su 4106) tagliate senza ordinamento, e la prova l'ha vista l'utente — della Scandinavia compaiono **9 locali su 33**. Con la crescita peggiora da sola: a 10× i dati sarebbe il 2,4%, cioè **più cresci più la mappa sembra vuota**. Nessuna manopola client lo tocca: il client può diradare solo ciò che è arrivato (il passo A, `feature/map-thinning`, ha chiuso il RENDERING e non questo).

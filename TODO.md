...
- [x] ~~**Thumbnail solo di quello che si vede**~~ — FATTO il 2026-09-25: `hooks/useRestaurantDetail.ts` scarica in anticipo solo le **prime 6** miniature (quelle del carosello «Foto dei clienti» all'apertura, nello stesso ordine); le altre quando entrano in scena. Costo per apertura scheda fisso a ~70 KB qualunque sia la copertura futura (prima fino a 522 KB). Le foto dei piatti della scheda non hanno prefetch: miniature quando compare il carosello, foto grande solo al tocco.
...
Il flusso (`services/storageService.ts`) è solido nei fondamentali: WEBP, compressione client, cleanup cascade, RLS policies, thumb 250px @ q.0.65 + full 600px @ q.0.7 quadrati (unico tipo rimasto dopo la rimozione delle foto menù: i preset `menu` non esistono più), prefetch delle prime 6 miniature in `useRestaurantDetail`. Restano margini per UX avanzata e resilienza.

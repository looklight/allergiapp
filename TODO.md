...
- [ ] **Far vedere gli errori di scrittura in interfaccia.** Dal 30/08 ogni scrittura riporta il suo errore in *console* (`reportError` in `lib/storage.ts`), ma l'utente non vede niente: l'interfaccia mostra il dato come salvato anche quando il server l'ha rifiutato. Serve l'avviso a schermo — e il posto naturale è l'indicatore "Salvataggio… / Salvato" accanto al titolo dell'editor, proposto e non ancora fatto Dal 30/08 il portale scrive su Supabase in modo ottimistico: l'interfaccia si aggiorna subito e la scrittura parte dietro. Se fallisce — rete assente, sessione scaduta, RLS — **non se ne accorge nessuno** finché non si ricarica la pagina e il dato non c'è più. Con dati di prova è tollerabile, col menù di un ristoratore no. Serve almeno un avviso che dica "non è stato salvato" e una riprova

  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: reportsData } = await supabase
        .from('reports')
        .select('*, profiles!user_id(username, is_anonymous), reviews!review_id(comment, rating, profiles!user_id(username)), menu_photos!menu_photo_id(thumbnail_url, image_url)')
        .eq('restaurant_id', id)
        .eq('status', 'pending')
        .order('created_at', { ascending: false });

...
CREATE OR REPLACE FUNCTION upsert_review(
  ...
  p_generated_id UUID DEFAULT NULL      -- ID pre-generato per nuove review (usato per path Storage)
)
RETURNS UUID AS $$
DECLARE
  v_review_id UUID;
BEGIN
  IF p_review_id IS NULL THEN
    -- Nuova review: usa ID pre-generato se fornito, altrimenti genera uno nuovo

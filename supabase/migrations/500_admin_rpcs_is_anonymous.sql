-- Migration 500: aggiunge is_anonymous alle RPC admin per utenti.
-- Permette alla dashboard di mostrare un badge "anonimo" su lista e scheda
-- utente, coerente con quanto già fatto in /reports.
--
-- Numerazione: prima migration admin-only del range 500+ (vedi memory
-- project-db-migrations-pattern). Le admin-only migrations partono ora da 500
-- per evitare collisioni di numero con `main`.
